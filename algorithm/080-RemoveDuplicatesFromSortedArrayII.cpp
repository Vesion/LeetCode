...
// Soluton 1 : tweak std::unique, call it two time to generate two unique subarray
...
int removeDuplicates_two_scans(vector<int>& nums) {
...
// Solution 2 : one scan, can be apply to occur times is 3, 4, ...
int removeDuplicates_one_scan(vector<int>& nums) {
    int n = nums.size();
    if (n <= 2) return n;
    int result = 2;
    for (int i = 2; i < n; ++i) {
...
int main() {
    ...
    cout << removeDuplicates_two_scans(a) << endl;
    cout << endl;
