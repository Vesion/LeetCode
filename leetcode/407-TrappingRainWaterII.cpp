...
public:
    int trapRainWater(vector<vector<int>>& heightMap) {
        ...
        priority_queue<S, vector<S>, greater<S>> q; // minimum heap, {height, pos = i*n+j}
        vector<vector<bool>> visited(m, vector<bool>(n, false));
        // enqueue 4 borders
        for (int i = 0; i < m; ++i) {
            q.push({heightMap[i][0], i, 0}); visited[i][0] = true;
            q.push({heightMap[i][n-1], i, n-1}); visited[i][n-1] = true;
        }
        for (int j = 1; j < n-1; ++j) {
            q.push({heightMap[0][j], 0, j}); visited[0][j] = true;
            q.push({heightMap[m-1][j], m-1, j}); visited[m-1][j] = true;
        }

        int res = 0;
        int go[4][2] = {{1,0}, {-1,0}, {0,1}, {0,-1}};
        while (!q.empty()) {
            auto [height, x, y] = q.top(); q.pop();
            for (int d = 0; d < 4; ++d) {
                int nx = x+go[d][0], ny = y+go[d][1];
                if (nx < 0 || nx >= m || ny < 0 || ny >= n || visited[nx][ny]) continue;
                visited[nx][ny] = true;
                res += max(0, height-heightMap[nx][ny]);
                q.push({max(height, heightMap[nx][ny]), nx, ny}); // fill the new cell with water if it is lower
            }
        }
        return res;
    }
