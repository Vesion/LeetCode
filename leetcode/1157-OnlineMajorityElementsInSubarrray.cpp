...
public:
    ...
    MajorityChecker(vector<int>& arr) {
        ...
        for (int i = 0; i < arr.size(); ++i) {
            // nums is arr without any duplicates
            if (indice[arr[i]].empty()) nums.push_back(arr[i]);
            indice[arr[i]].push_back(i);
        }
        // sort nums according to num frequency
        sort(nums.begin(), nums.end(),
             [this](int i, int j) {
                 return indice[i].size() > indice[j].size();
             });
    }

    int query(int left, int right, int threshold) {
        // try num one by one starting from most frequent one
        for (int num : nums) {
            const auto& ni = indice[num];
            if (ni.size() < threshold) break;  // nice pruning
